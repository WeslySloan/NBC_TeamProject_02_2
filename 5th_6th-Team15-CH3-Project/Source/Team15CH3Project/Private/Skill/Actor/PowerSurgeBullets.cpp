...
APowerSurgeBullets::APowerSurgeBullets()
{
    ...
    CollisionComp->OnComponentBeginOverlap.AddDynamic(this, &APowerSurgeBullets::OnOverlapBegin);
...
void APowerSurgeBullets::BeginPlay()
{
    ...
    if (MyOwner)
    {
        ...
        {
            AActor* Indicator = IndicatorComp->GetSpawnedIndicatorActor();
            FireDir = Indicator->GetActorRightVector();

            UE_LOG(LogTemp, Warning, TEXT("[PowerSurgeBullets] Using Indicator RightVector: %s"), *FireDir.ToString());
        }
        else
        {
...
void APowerSurgeBullets::OnOverlapBegin(
